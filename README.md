...
Позволяет описать процесс «как есть» (AS-IS) и целевое состояние с ИИ (TO-BE), учесть риски, динамику раскатки, OPEX/CAPEX и получить наглядный расчёт окупаемости.

## Статус проекта

**MVP задеплоен.** Фазы 1–13 реализованы. Проект доступен по адресу: **http://89.23.112.115**
...
| 10: Авторизация и переделка шаблонов | ✅ Завершена |
| 11: Экспорт результатов в PDF | ✅ Завершена |
| 12: Переработка раздела Риски | ✅ Завершена |
| 13: Шаринг и экспорт XLSX | ✅ Завершена |
...
│   ├── migrate-errors.ts   # Идемпотентный скрипт миграции ErrorItem (Фаза 12)
...
| GET | `/api/export/pdf/:id` | Генерация PDF-отчёта через Puppeteer (`?horizon=12|24|36`) |
