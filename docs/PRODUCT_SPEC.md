...
3. Пользователь описывает процесс в текстовом поле (Ctrl/Cmd+Enter — быстрый запуск)
4. Запускается двухшаговая цепочка: генерация → верификация (прогресс отображается)
5. Отображается превью результата: читаемая таблица + пояснение ИИ
6. Пользователь нажимает «Применить» (заменяет данные) или «Перегенерировать»

### 11.3 Двухшаговая цепочка промптов

| Шаг | Модель | Роль |
|-----|--------|------|
| 1. Генерация | `google/gemini-3.1-pro-preview` | Формирует список этапов/ошибок/затрат по описанию |
| 2. Верификация | `openai/gpt-oss-120b:free` | Проверяет реалистичность, логику, полноту; исправляет |

Оба запроса выполняются на сервере через API-роут `/api/ai/generate` — ключ OpenRouter никогда не покидает серверную сторону.
