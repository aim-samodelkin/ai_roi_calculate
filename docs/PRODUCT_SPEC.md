...
| Удельное время | computed | = Время в часах × Вероятность |
...
> **Принцип:** Дни взвешиваются по доле выполнения — аналогично `unitTime = timeHours × executionShare` и `unitCost = stepCost × executionShare`. Это даёт ожидаемую длительность цикла на одну операцию, а не максимально возможную.
