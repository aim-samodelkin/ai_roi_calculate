...
Таблица рисков текущего процесса с перетаскиванием для изменения порядка:

| Поле | Тип | Описание |
|------|-----|----------|
| Тип риска | string | Описание риска |
| Этап процесса | string | На каком этапе возникает |
| Частота (доля случаев) | number (0-1) | Как часто возникает риск |
| Стоимость исправления | number (₽) | Сколько стоит исправить один риск |
| Время на исправление (ч) | number | Время на исправление одного риска |
| Удельная стоимость риска | computed (₽) | = Частота × Стоимость исправления |
| Удельное время риска | computed | = Частота × Время на исправление |

### 4.4 Риски TO-BE

Аналогично рискам AS-IS (включая перетаскивание строк). Кнопка «Скопировать из AS-IS» копирует все строки AS-IS в TO-BE одним кликом — с учётом того, что ИИ снижает частоту и/или стоимость рисков. Кнопка «Заполнить с ИИ» автоматически формирует реалистичную картину остаточных рисков TO-BE на основе AS-IS рисков и целевого процесса.
...
Экономия на рисках (за 1 операцию) =
  Σ Удельная стоимость рисков AS-IS − Σ Удельная стоимость рисков TO-BE
