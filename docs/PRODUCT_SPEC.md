...
**AI ROI Calculator** — веб-сервис, позволяющий оценить экономическую эффективность внедрения ИИ-решения в бизнес-процесс. Пользователь описывает текущий процесс (AS-IS) и целевой (TO-BE), риски, затраты на внедрение — и получает расчёт ROI с графиками динамики окупаемости.
...
- **ИИ-ассистент** — генерация этапов, рисков и затрат по текстовому описанию с верификацией
...
- Использует ИИ-ассистент для автоматической генерации этапов, рисков и затрат
...
- Доступ к **настройкам аккаунта** (`/my/settings`) — справочник должностей с ценами часа для автодополнения в расчётах
...
├── Шаг 3: Риски AS-IS (таблица рисков)
├── Шаг 4: Риски TO-BE (таблица рисков)
...
| Удельный срок | computed | = Календарный срок в днях × Доля в выполнении |
| Удельная стоимость | computed (₽) | = Стоимость этапа × Доля в выполнении |

**Итого по процессу:**
- Суммарный удельный срок (Σ Дней × Доля)
- Суммарная удельная стоимость
- Сумма Дней в строке итогов не показывается
...
Таблица рисков текущего процесса с перетаскиванием для изменения порядка. Структура аналогична таблице этапов процесса:

| Поле | Тип | Описание |
|------|-----|----------|
| Тип риска | string | Описание риска |
| Сотрудник | string | Роль/должность того, кто устраняет риск |
| Цена часа | number (₽) | Стоимость часа работы сотрудника |
| Время в часах | number | Трудозатраты на устранение одного случая |
| Календарный срок в днях | number | Длительность устранения в календарных днях |
| Доп. затраты | number (₽) | Прямые затраты при возникновении риска (штрафы, переделка и пр.) |
| Стоимость риска | computed (₽) | = Цена часа × Время в часах + Доп. затраты |
| Вероятность | number (0-1) | С какой долей случаев возникает риск |
| Удельная стоимость | computed (₽) | = Стоимость риска × Вероятность |
| Удельный срок | computed | = Календарный срок в днях × Вероятность |

**Итого по рискам:**
- Суммарный удельный срок (Σ Дней × Вероятность)
- Суммарная удельная стоимость
- Сумма Дней в строке итогов не показывается

### 4.4 Риски TO-BE

Аналогично рискам AS-IS (включая перетаскивание строк). Кнопка «Скопировать из AS-IS» копирует все строки AS-IS в TO-BE одним кликом — с учётом того, что ИИ снижает вероятность и/или стоимость устранения рисков. Кнопка «Заполнить с ИИ» автоматически формирует реалистичную картину остаточных рисков TO-BE на основе AS-IS рисков и целевого процесса.
...
Экономия на рисках (за 1 операцию) =
  Σ (Стоимость риска × Вероятность) AS-IS − Σ (Стоимость риска × Вероятность) TO-BE

Общая экономия за 1 операцию =
  Экономия на процессе + Экономия на рисках

Экономия времени за 1 операцию =
  (Удельное время AS-IS + Удельное время рисков AS-IS)
  − (Удельное время TO-BE + Удельное время рисков TO-BE)
```
...
Ускорение процесса (календарные дни) =
  Ожидаемый срок цикла AS-IS / Ожидаемый срок цикла TO-BE
  где Ожидаемый срок = Σ (calendarDays × executionShare) по этапам
...
### 5.5 Анализ сроков (календарные дни)

Отдельный блок на экране результатов — сравнение процессов по календарным срокам (не трудозатратам). Показывается только если заполнено хотя бы одно поле «Дни» в AS-IS или TO-BE.

```
Длительность процесса AS-IS = Σ (calendarDays × executionShare) по ProcessStep AS-IS
Длительность процесса TO-BE = Σ (calendarDays × executionShare) по ProcessStep TO-BE

Задержки из-за рисков AS-IS = Σ (calendarDays × frequency) по ErrorItem AS-IS
Задержки из-за рисков TO-BE = Σ (calendarDays × frequency) по ErrorItem TO-BE

Общий цикл AS-IS = Длительность процесса AS-IS + Задержки рисков AS-IS
Общий цикл TO-BE = Длительность процесса TO-BE + Задержки рисков TO-BE

Экономия по каждой метрике = AS-IS − TO-BE (в днях и %)
```

> **Принцип:** Дни взвешиваются по доле выполнения — аналогично `unitCalendarDays = calendarDays × executionShare` (процессы) и `unitCalendarDays = calendarDays × frequency` (риски). Это даёт ожидаемую длительность цикла на одну операцию, а не максимально возможную.

Три карточки:
1. **Длительность процесса** — суммарный календарный срок этапов
2. **Задержки из-за рисков** — ожидаемые задержки с учётом вероятности (скрывается если нет данных)
3. **Общий цикл операции** — процесс + риски

Каждая карточка содержит визуальную полосу сравнения (AS-IS красная → TO-BE зелёная), абсолютную и процентную экономию. Блок также включается в PDF-отчёт.
...
- **Столбец 2:** Экономия на рисках (зелёный)
...
- **Тип:** Stacked bar chart
- **Сравнение:** Стоимость, время и календарный срок на операцию — до и после
- **Включает:** Процесс + риски по каждой метрике
- **Подграфики:**
  - Стоимость на операцию, ₽
  - Время на операцию, ч
  - Календарный срок, дн. (показывается только если есть данные по дням)
...
- Риски AS-IS и TO-BE (со всеми полями)
...
- На шагах ввода (Процесс AS-IS, TO-BE, Риски AS-IS, TO-BE, CAPEX, OPEX):
...
- На шаге «Результаты» — ключевые цифры + графики:
  ...
  - Блок «Анализ сроков»: карточки с сравнением AS-IS vs TO-BE по календарным дням (процесс, задержки рисков, общий цикл) с визуальными полосами и экономией в днях и % (см. п. 5.5)
  - Детализация экономии по категориям (процесс / риски)
  - Четыре графика: динамика окупаемости, помесячная экономия, кривая раскатки, сравнение AS-IS vs TO-BE (с подграфиком по календарным дням)
...
- Перетаскивание строк (drag & drop) для изменения порядка этапов, ошибок, статей CAPEX и OPEX — захват только за иконку ⠿ и номер строки в левой части
- Кнопка «Скопировать из AS-IS» на шагах TO-BE (процесс и риски)
...
### 9.6 Настройки аккаунта (`/my/settings`, только для авторизованных)

- Справочник должностей: таблица с inline-редактированием (название, цена часа ₽)
- Кнопка «+ Добавить должность»
- Автосохранение по debounce (500ms)
- Доступ: пункт «Настройки аккаунта» в дропдаун-меню пользователя в шапке сайта (иконка ⚙)
- Должности используются для автодополнения поля «Сотрудник» в таблицах процессов и рисков — при выборе автоматически подставляется и имя, и ставка

### 9.7 Управление шаблонами (`/templates/manage`, только ADMIN)
...
На каждой из 6 вкладок ввода калькулятора (Процесс AS-IS, TO-BE, Риски AS-IS, TO-BE, CAPEX, OPEX) доступна кнопка **«Заполнить с ИИ»**. Пользователь описывает процесс свободным текстом — ИИ генерирует структурированные данные и предлагает их для подтверждения.
...
| Риски AS-IS | Шаги AS-IS, существующие риски AS-IS |
| Риски TO-BE | Риски AS-IS, шаги TO-BE, существующие риски TO-BE |
