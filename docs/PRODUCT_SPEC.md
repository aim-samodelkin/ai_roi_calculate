...
Структура полностью аналогична AS-IS. Предзаполняется копией AS-IS при переходе на шаг, если пуста.

### 4.3 Ошибки AS-IS

Таблица ошибок текущего процесса:
...
Аналогично ошибкам AS-IS — но с учётом того, что ИИ снижает частоту и/или стоимость ошибок.
...
- Предзаполненные данные:
  - Этапы процесса AS-IS (без цен)
  - Этапы процесса TO-BE (без цен)
  - Типы ошибок AS-IS (без стоимостей)
  - Типы ошибок TO-BE (без стоимостей)
  - Статьи CAPEX (без сумм)
  - Статьи OPEX (без сумм)
  - Модель раскатки по умолчанию

### При загрузке шаблона
Пользователь получает предзаполненный расчёт, в котором нужно только подставить свои стоимости, часовые ставки и объёмы.
...
- Кнопка «Сохранить» (генерирует уникальную ссылку)
