...
export const ERROR_ITEM_SCHEMA = `{
  "items": [
    {
      "name": "string — тип / описание риска",
      "employee": "string — роль или должность сотрудника, который устраняет риск",
      "hourlyRate": "number — стоимость часа работы этого сотрудника в рублях (0 если неизвестна)",
      "timeHours": "number — время на исправление одного случая риска в часах",
      "calendarDays": "number — календарный срок устранения в днях (обычно ≥ timeHours / 8)",
      "extraCost": "number — прямые доп. затраты на исправление (штрафы, переработка, внешние услуги) в рублях (0 если нет)",
      "frequency": "number — вероятность возникновения риска от 0 до 1 (например 0.1 = 10% случаев)"
    }
  ],
...
{{jsonSchema}}`,
  ...
  errors_asis: {
    generation: `Ты эксперт по анализу качества бизнес-процессов. Твоя задача — выявить типичные риски и сбои в текущем процессе (AS-IS).

Расчёт называется: «{{calcName}}»
{{existingData}}{{contextBlock}}

Требования к каждому риску:
- name: конкретное описание риска или типа сбоя
- employee: роль сотрудника, который устраняет риск (например «Менеджер по качеству», «Оператор»)
- hourlyRate: оставь 0 (пользователь подставит свои ставки)
- timeHours: реалистичное время на исправление одного случая (часы)
- calendarDays: срок устранения в календарных днях (обычно ≥ timeHours / 8)
- extraCost: прямые затраты при возникновении риска (штрафы, переделка, внешние услуги), 0 если нет
- frequency: вероятность возникновения от 0 до 1 (0.05–0.30 типично)
- Не дублируй уже существующие риски
...
Критерии проверки для рисков AS-IS:
1. frequency реалистична: 0.01–0.5 (риски с вероятностью > 50% — исключение)
2. timeHours > 0 (устранение риска всегда требует времени)
3. calendarDays ≥ timeHours / 8
4. employee указывает на реальную роль
5. Нет дублирующих рисков
6. Не более 10–15 рисков (качество важнее количества)
...
{{jsonSchema}}`,
  ...
  errors_tobe: {
    generation: `Ты эксперт по оценке влияния ИИ на качество процессов. Твоя задача — показать, как ИИ изменит картину рисков TO-BE.

Расчёт называется: «{{calcName}}»
{{existingData}}{{contextBlock}}

Правила:
- Для каждого риска AS-IS реши: исчезает ли он, снижается ли вероятность или время/стоимость исправления
- ИИ редко устраняет риски полностью — как правило снижает вероятность на 30–80%
- Новые типы рисков, специфичные для ИИ-системы: добавь (галлюцинации ИИ, сбои интеграции, ошибки разметки)
- timeHours и extraCost должны быть реалистичными для оставшихся рисков
...
Ниже — JSON, сгенерированный на предыдущем шаге. Проверь его и верни исправленную версию в том же формате.
{{asisContext}}
Критерии проверки для рисков TO-BE:
1. frequency TO-BE ≤ frequency AS-IS (ИИ не увеличивает вероятность рисков для тех же типов)
2. Ни один риск не должен иметь frequency = 0 (если риск в TO-BE, он ненулевой)
3. timeHours > 0
4. calendarDays ≥ timeHours / 8
5. Новые риски ИИ (сбои, галлюцинации) логичны для данного контекста
6. Снижение вероятности не должно быть > 90% для всех рисков сразу
...
function errorsToText(items: ErrorItem[]): string {
  ...
  return items
    .map(
      (e, i) =>
        `${i + 1}. ${e.name} (сотрудник: ${e.employee || "—"}, время: ${e.timeHours} ч, вероятность: ${Math.round(e.frequency * 100)}%)`
    )
    .join("\n");
}
